...
const Navbar = () => {
    ...
    return (
        <nav style={{background: ""}} className=" bg-indigo-200 shadow-lg w-full md:py-1 py-2 z-50 fixed top-0 left-0 md:text-sm">
                    ...
                    <ul className="justify-end items-center space-y-6 md:flex md:space-x-6 md:space-y-0">
                        {
                            navigation.map((item, index) => {
                                return (
                                    <li key={index} className=" hover:text-indigo-600 text-lg">
                                        <NavLink to={item.path} onClick={()=>setState(false)} className="block">
                                            <p>
